...
def signup(payload: SignupBody, db: Session = Depends(get_db)):
    ...
    # Set subscription_expiry to 30 days from now for free trial (can be extended upon payment)
    now = datetime.now(timezone.utc).replace(tzinfo=None)
...
def login(payload: LoginBody, db: Session = Depends(get_db)):
    ...
    # Ensure user has a valid subscription expiry (for legacy accounts or free tier)
